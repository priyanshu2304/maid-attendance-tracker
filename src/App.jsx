...
const T = {
  ...
  hi: {
    ...
    delete: "हटाएँ",
    confirmDelete:
      "क्या आप इस स्टाफ को हटाना चाहते हैं? उनकी उपस्थिति भी हट जाएगी।",
    dataStay:
      "डेटा इसी डिवाइस में रहता है (लोकल स्टोरेज)। ऑफ़लाइन काम करता है।",
  },
};
...
const rangeDates = (start, end) => {
  ...
  for (let t = new Date(ds); t <= de; t.setDate(t.getDate() + 1))
    out.push(fmt(t));
  return out;
};
...
export default function App() {
  ...
  const [start, setStart] = useState(
    fmt(new Date(Date.now() - 6 * 24 * 3600 * 1000))
  );
  ...
  function setPresent(id, date, present) {
    setAtt((prev) => ({
      ...prev,
      [id]: { ...(prev[id] || {}), [date]: present || undefined },
    }));
  }
  function markAll(date) {
    const copy = { ...att };
    for (const s of staff) {
      copy[s.id] = { ...(copy[s.id] || {}), [date]: true };
    }
    setAtt(copy);
  }
          ...
          <div className="card" style={{ marginBottom: 10 }}>
            ...
            {staff.length > 0 && (
              <div className="bar" style={{ marginTop: 10 }}>
                <button className="btn primary" onClick={() => markAll(date)}>
                  {t.allPresent}
                </button>
                <button
              ...
              <div style={{ marginTop: 10 }}>
                <div className="item">
                  ...
                  <div style={{ textAlign: "right" }}>
                    <div>
                      {t.daysPresent}: <b>{report.presentDays.length}</b>
                    </div>
                    <div>
                      {t.totalWage}: <b>₹{report.total}</b>
                    </div>
                  </div>
                </div>
                <div
                ...
                >
                  {report.presentDays.length === 0 ? (
                    ...
                    report.presentDays.map((d) => (
                      ...
                      >
                        <div>{d}</div>
                        <div className="pill ok">{t.present}</div>
                      </div>
                    ))
                  )}
                </div>
